...
<head>
    ...
    <style>
        /* Styling as provided in your original code */
        body {
...
<body>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const seatPrices = { classic: 450, recliner: 750 };

            const renderSeats = (type) => {
                const container = document.querySelector(`#${type}-seats`);
            ...
            const updateTotal = () => {
                const total = selectedSeats.reduce((sum, seat) => sum + seatPrices[seat.type], 0);
            ...
            seats.forEach((seat) => {
                seat.addEventListener("click", () => {
                    ...
                    const seatData = {
                        row: seat.parentNode.querySelector(".row-label").textContent,
                        number: seat.textContent,
                        type: seat.classList.contains("recliner") ? "recliner" : "classic",
                    };
            ...
            confirmButton.addEventListener("click", () => {
                ...
                const total = selectedSeats.reduce((sum, seat) => sum + seatPrices[seat.type], 0);
                const url = `booking_summary.php?selectedSeats=${encodeURIComponent(selectedSeatsString)}&total=${total}`;
